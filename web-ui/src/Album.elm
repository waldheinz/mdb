...
withListFilter flt m =
    if (flt == m.albumFilter)
        then (m, Listing.refresh m.albums |> Cmd.map AlbumListing)
            ...
            let
                (as_, afx)  = Listing.withFetchTask (Server.fetchAlbums flt m.order m.direction) m.albums
            in
                ( { m | albumFilter = flt, albums = as_ }, Cmd.map AlbumListing afx )
...
listPagination m = Html.map AlbumListing (Listing.pagination m.albums)

updateList : ListAction -> ListModel -> (ListModel, Cmd ListAction)
updateList a m = case a of
    AlbumListing la -> Listing.update la m.albums |> \(as_, fx) -> ( { m | albums = as_ }, Cmd.map AlbumListing fx )
